...
import { createHouse } from "../../store/housesSlice";
import Button from "../../components/ui/Button";
import Input from "../../components/ui/Input";
import Textarea from "../../components/ui/Textarea";
import { AMENITIES, ROUTES } from "../../constants/index";
...
const AddHouse = () => {
  ...
  const { actionLoading } = useSelector((s: RootState) => s.houses);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (intervalRef.current) clearInterval(intervalRef.current);
      setProgress(0);
      setError((err as string) ?? "Failed to add house. Please try again.");
    }
  };
      ...
      <div className="max-w-lg mx-auto">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-wrap gap-2">
              {AMENITIES.map(({ value, label }) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${form.amenities.includes(value) ? "bg-blue-600 text-white border-blue-600" : "border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:border-blue-400"}`}
                >
                  {label}
                </button>
              ))}
          ...
          <div>
            ...
            <label
              className={`flex flex-col items-center justify-center w-full h-28 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${files.length >= 2 ? "border-gray-200 dark:border-gray-700 opacity-50 cursor-not-allowed" : "border-gray-300 dark:border-gray-600 hover:border-blue-400 bg-gray-50 dark:bg-gray-900/30"}`}
            >
