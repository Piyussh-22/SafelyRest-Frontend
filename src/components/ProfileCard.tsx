...
const ProfileCard = () => {
  ...
  return (
    <div className="w-72 h-96 perspective relative">
      <div
        className={`relative w-full h-full transition-transform duration-500 transform-style preserve-3d ${isMobile && flipped ? "rotate-y-180" : "hover:rotate-y-180"} ${!isMobile ? "hover:scale-105" : flipped ? "scale-105" : "scale-100"} transition-transform duration-300`}
      >
        <div
          className={`absolute w-full h-full backface-hidden rounded-xl overflow-hidden shadow-lg transition-all duration-300 ${isMobile && flipped ? "filter blur-sm" : ""}`}
        >
          <img
            src="/piyush.webp"
            alt={name}
